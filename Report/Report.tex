...
%(Paul Smith. Waterfall model (1).svg [Internet]. Wikipedia; 08.03.2008 [09.08.2011; 09.09.2013]. Available from: %upload.wikimedia.org/wikipedia/commons/0/06/Waterfall_model_%281%29.svg )
...
%(Royce WW. Managing the development of large software systems [Internett]; 1970 [09.09.2013]. Availiable from: %leadinganswers.typepad.com/leading_answers/files/original_waterfall_paper_winston_royce.pdf )
...
\subsection{Overview}
The rest of this SRS document describes the proposed product in the means of its desired functionality, characteristics, constraints and perspectives. This general description could be found Overall description. Description of the actual requirements is to be found in Specific requirements .
...
Our key quality attribute, usability, relates tightly to user interfaces. For this reason, all usability-related requirements on user interface are listed in section Software system attributes as quality attribute requirements.
...
\subsubsection{Unavailability of connection}
...
See chapter Usability for requirements on usability.

\subsection{Constraints}

\subsubsection{HTML5 use}
Since use on many different devices and platforms is expected and necessary, customer requested that the frontend would be a web application. As the system is crowdsourced and heavily relies on users, it it necessary that it interacts very well with the users and is visually appealing. Also, for reasons stated in Unavailability of connection, local storage is necessary for enabling offline mode (in the future). This implies the use of cutting-edge technologies for the development.
...
\hline
F2.4 & System shall let users like and share wall over social networks (facebook, twitter). See Social networks for details
...
\hline
F3.4 & System shall let users like and share stories over social networks (facebook, twitter). See Social networks for details\\
\hline
...
Quality attribute scenario shall to evaluate the system shall be determined later. TBD

\subsubsection{Usability}
From the description of typical usage (mentioned in User characteristics) and characteristic problems listed in Web application front end, we came up with several usability scenarios that shall evaluate the usability quality of the software system. As usability is a big concern in this project, usability scenarios and tests shall be included in a separate part of the report, outside the SRS.
...
Division between logic and presentation makes the system more modular and
